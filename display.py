...
class SSD1306_DualSPI(framebuf.FrameBuffer):
    ...
    def show(self):
        for page in range(self.pages):
            left = page * 256
            right = left + 128
            for cs, start in ((self.cs1, left), (self.cs2, right)):
                self.write_cmd(0xB0 | page, cs)
                self.write_cmd(0x00, cs)
                self.write_cmd(0x10, cs)
                self.write_data(self.buffer[start:start + 128], cs)
